...
class ForecastViewModel: ObservableObject {
    var weatherService: API!
    var cityName: String = "Rabat"
    
    
        @Published var weatherForecast = ForecastWeatherResponse()
        
        init() {
            self.weatherService = API()
        }
        ...
        var weatherDay: String{
            if let day = self.weatherForecast.list?.first?.dt {
                let formattedDay = first().timeConverter(timeStamp: Int(day), isDay: false)
        ...
        var sunrise: String{
            if let sunrise = self.weatherForecast.city?.sunrise {
                return first().hourFormatter(timeStamp: Int(sunrise))
            } else{
                return ""
            }
        }
        ...
        var sunset: String{
            if let sunset = self.weatherForecast.city?.sunset {
                return first().hourFormatter(timeStamp: Int(sunset))
            } else{
                return ""
            }
        }
